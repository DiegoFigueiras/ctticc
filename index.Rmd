...
The visual representations are most informative when item characteristics are plotted simulataneously:
...
The test administrator receives visual feedback that `item 7` is the easiest, .difficult[item 1] is the most difficult, `item 4` and `item 6` are not  discriminating well, and something is "wrong" with `item 3`. 
...
The order of administration of `item 1` and `item 7` should be swapped. `Item 4` and `item 6` should be evaluated for *relevance to the broader test content*. `Item 3` is either a scoring key mistake or should be dropped. 
...
# For Sakai 
<img src="https://cdn.elearningindustry.com/wp-content/uploads/2022/12/a3ef8c2c97b3d8c819f5a2d5bce9a0db.png" width="220" height="200"  
style="position:absolute; right:400px; bottom:100px;">


We would like to use these visuals for improving learning management systems. These visuals could be incorporated into either the "test & Quizzes" page, in Instructor View, or the "Statistics" page.
...
.pull-left[More detailed information is available in our write-up (click on image to right)]
